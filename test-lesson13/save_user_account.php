...
if (!empty($myrow['login']) && $myrow['login']!=$oldLogin) {   
   echo "Извините, введённый вами логин уже зарегистрирован. Введите другой логин.<br><br>";     
   echo '<a href="/user_account.php">Вернуться для изменения данных</a>';
   exit;
} else {
//    $_SESSION['login']=$myrow['login']; 
//    $_SESSION['id']=$myrow['user_id'];
}
// обновление всех полей у $oldLogin
    $result2 = "UPDATE users SET login='$login', password='$password', email='$email', tel='$tel' WHERE login='$oldLogin'";
    mysqli_query($connect, $result2);   

if ($result2== true){
   echo "Вы успешно изменили данные! Теперь вы можете зайти на сайт. <a href='index.php'>Главная страница</a>";
   $sql_c = "SELECT * FROM users WHERE login='$login'";
   $query_c = mysqli_query($connect, $sql_c);
   $myrow = mysqli_fetch_assoc($query_c);
   $_SESSION['login']=$myrow['login']; 
   $_SESSION['id']=$myrow['user_id'];
    ...
    echo "Ошибка! Вы не изменили данные.";    
    echo "Вернуться в Личный кабинет пользователя<br><br>" ;
    echo '<a href="/user_account.php">Личный кабинет пользователя</a>';
}
?>